...
void ArUco::drawBoard(float boardSize){
	glColor3f(0,0,0);
	glBegin(GL_LINES);
	glVertex3f(boardSize/3, boardSize*2/3, 0.0f);
	glVertex3f(boardSize/3, boardSize*8/3, 0.0f);
	glEnd();
	...
	glBegin(GL_LINES);
	glVertex3f(boardSize, boardSize*4/3, 0.0f);
	glVertex3f(-boardSize, boardSize*4/3, 0.0f);
	glEnd();

	glBegin(GL_LINES);
	glVertex3f(boardSize, boardSize*2, 0.0f);
	glVertex3f(-boardSize, boardSize*2, 0.0f);
	glEnd();
...
void ArUco::drawScene() {
   ...
   for (unsigned int m=0;m<m_Markers.size();m++)
   {
	  ...
	  Point2f center = m_Markers.at(m).getCenter();
	  //Drawing board 
	  if (m == 0){
	 	  drawBoard(m_MarkerSize*3);
	  }
	  else if (m%2 == 1){
		  glColor3f(1, 0, 0);
		  ...
		  /*int i = 1;
		  if (center.x - m_Markers.at(m-2).getCenter().x < m_MarkerSize && center.y - m_Markers.at(m-2).getCenter().y < m_MarkerSize){
			  glBegin(GL_LINES);
			  glVertex3f(center.x, center.y, 0);
			  glVertex3f(m_Markers.at(m-2).getCenter().x, m_Markers.at(m-2).getCenter().y, 0);
			  glEnd();
			  i++;
		  }*/
	  }
		  ...
		  glTranslatef(0, 0, m_MarkerSize/2);
		  glPushMatrix();
		  glutSolidSphere(m_MarkerSize/2,15,15);
	  }
